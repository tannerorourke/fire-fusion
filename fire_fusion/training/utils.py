...
def estimate_model_size_mb(model: torch.nn.Module) -> float:
    """ Parameter memory in MB at 4 bytes per fp32 parameter; buffers and
        optimizer state not counted. """
    return sum(p.numel() for p in model.parameters()) * 4 / 1024 / 1024


def set_global_seed(seed: int):
    """ Seed numpy, random, and torch (CPU and CUDA), and force deterministic,
        non-benchmarked cuDNN kernels.
    """
    np.random.seed(seed)
    random.seed(seed)
...
def checkpoint_name(experiment: str) -> str:
    """ Checkpoint base name for an experiment, '<experiment>_model'. """
    return f"{experiment}_model"


def get_device_config(maximum: int | None = None, utilization: float | None = 0.75):
    """ Pick CUDA or CPU, enable TF32 matmul and cuDNN, and return the device with a
        worker count sized from CPU count, `utilization`, and `maximum`.
    """
...
def save_model(
    ...
    """ Save the model's state dict under MODEL_SAVE_DIR, either overwriting
        `<name_base>.th` or writing the next `<name_base>_<i>.th`, and return the
        saved path.
    """
    ...
    # -- a torch.compile wrapper prefixes keys with '_orig_mod.'; the plain
    #    module is saved, loadable in uncompiled processes
...
def export_to_b2(
    *paths: str | Path,
    ...
    """ Upload each path to B2 under `prefix`: a file lands at `<prefix>/<filename>`,
        a directory recursively under its own relative paths, using credentials from
        the B2_* environment. Return whether the upload succeeded.
    """
    try:
        ...
        # -- the weights are already on local disk; a credentials or network
        # -- failure here does not take down a complete run
...
def load_model(
    ...
    """ Load a state dict into `model`. A bare name resolves under MODEL_SAVE_DIR,
        mirroring save_model's layout. strict=False tolerates a checkpoint covering
        part of the model, such as a backbone loaded into freshly initialized heads,
        and the return value lists whichever keys were missing or unexpected.
    """
    p = Path(path)
    if p.parent == Path("."):
        p = MODEL_SAVE_DIR / p
...
def save_calibration(params: dict, name_base: str = "model") -> str:
    """ Save calibration params as JSON under `<name_base>.calib.json`, alongside
        the checkpoint of the same name, and return the saved path.
    """
    MODEL_SAVE_DIR.mkdir(parents=True, exist_ok=True)
...
def load_calibration(name_base: str = "model") -> dict | None:
    """ Load calibration params from JSON. A bare name resolves under MODEL_SAVE_DIR;
        a path ending in `.calib.json` is read as given. Return None if no sidecar exists.
    """
    p = Path(name_base)
    if p.suffix != ".json":
...
class WarmupCosineAnnealingLR:
    ...
    def __init__(self,
        ...
        """ Build a LinearLR warmup feeding into CosineAnnealingLR via SequentialLR,
            warming up at full scale when `base_lr` is 0.
        """
